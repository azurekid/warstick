...
if [ -z "$USB_ROOT" ] || [ ! -d "$USB_ROOT/command-core" ]; then
    echo "WarStick runtime root was not provided. Start using run-linux.sh."
    exit 1
fi
...
trap "stop_image_server; stop_history_server; stop_engine; cleanup_runtime_files; exit" SIGINT SIGTERM
if ! start_engine "linux"; then
    echo -e "${RED}[!] Unable to start the neural engine. The command console will not open.${RESET}"
    stop_engine
    cleanup_runtime_files
    exit 1
fi
start_history_server || true
start_image_server "linux" || true
...
EOF
                ...
                query_llm_with_live_animation "$PAYLOAD"
                RAW_RESPONSE=$(cat "$USB_ROOT/command-core/last_response.json" 2>/dev/null)
                CHAT_RESPONSE=$(extract_chat_response "$RAW_RESPONSE")

                if [ -z "$CHAT_RESPONSE" ]; then
                    CHAT_RESPONSE="The model did not return a response."
                fi

                echo -e "\n${CYAN}--- ASSISTANT RESPONSE ---${RESET}"
                echo "$CHAT_RESPONSE"
                echo -e "\n-------------------------------------------------------------------------------\n[ASSISTANT RESPONSE] $(date +'%H:%M:%S')\n$CHAT_RESPONSE\n===============================================================================" >> "$SESSION_LOG"
                echo -e "\n-------------------------------------------------------------------------------\n[ASSISTANT RESPONSE] $(date +'%H:%M:%S')\n$CHAT_RESPONSE\n===============================================================================" >> "$MASTER_LOG"
