...
if [ -z "$USB_ROOT" ] || [ ! -d "$USB_ROOT/command-core" ]; then
    echo "WarStick runtime root was not provided. Start using run-mac.command."
    exit 1
fi
...
source "$USB_ROOT/command-core/lib/ui_common.sh"

# ─── BACKGROUND ENGINE INITIALIZATION ───────────────────────────
start_engine "mac"
start_history_server || true
start_image_server "mac" || true
trap "stop_image_server; stop_history_server; stop_engine; exit" SIGINT SIGTERM
...
while true; do
    ...
    echo -e "  [4] Tactical Modules & External OSINT Lookups"
    ...
    case $CHOICE in
        1)
            ...
            cat "$USB_ROOT/command-core/prompts.txt" 2>/dev/null || echo -e "${ORANGE}[!] Run option 2 first to seed database structure.${RESET}"
            echo -e "\n${PINK}───────────────────────────────────────────────────────${RESET}"
        ...
        2)
            draw_banner
            if [ ! -f "$USB_ROOT/command-core/prompts.txt" ]; then
                mkdir -p "$USB_ROOT/command-core"
                echo " [1] Host Recon: Enumerate network interface adapters, local routing metrics, and scan for active listeners on 22 or 445." > "$USB_ROOT/command-core/prompts.txt"
                echo " [2] PrivEsc Audit: Check user assignment privileges, environment fields, and look for misconfigured system binaries." >> "$USB_ROOT/command-core/prompts.txt"
                echo " [3] Security Audit: Inspect host native packet firewall status profiles and review security update histories." >> "$USB_ROOT/command-core/prompts.txt"
                echo " [4] Persistence Check: Identify cron configurations, background daemons, and tasks loaded at boot initialization." >> "$USB_ROOT/command-core/prompts.txt"
            fi

            echo -e "${CYAN}─── [EXPLOIT & RECON VECTORS] ──────────────────────────${RESET}\n"
            cat "$USB_ROOT/command-core/prompts.txt"
            echo ""
            echo -ne "${ORANGE}Select target attack vector (1-4): ${RESET}"
            read NUM

            USER_TASK=$(grep "\[$NUM\]" "$USB_ROOT/command-core/prompts.txt" | sed "s/\[$NUM\] //")
            if [ -z "$USER_TASK" ]; then
            ...
            SESSION_LOG="$USB_ROOT/warstick-logs/audit_${TIMESTAMP}.txt"
            MASTER_LOG="$USB_ROOT/warstick-logs/warstick.log"

            SESSION_HEADER="===============================================================================\n[WARSTICK AUDIT SESSION - ${TIMESTAMP}]\nHost Platform : macOS (Darwin $(uname -r))\nTarget Vector : $USER_TASK\n==============================================================================="

            echo -e "$SESSION_HEADER" > "$SESSION_LOG"
            echo -e "\n$SESSION_HEADER" >> "$MASTER_LOG"

            CURRENT_CONTEXT="Host operating system is macOS (Darwin). The primary objective is: $USER_TASK. Generate valid native macOS / BSD terminal utilities only."
...
EOF
                ...
                RAW_RESPONSE=$(cat "$USB_ROOT/command-core/last_response.json" 2>/dev/null)
                ...
                CURRENT_CONTEXT="Host OS is macOS. Objective: $USER_TASK. Previous execution was '$EXEC_COMMAND' which returned: $CLEAN_OUTPUT. Determine the next optimal native macOS CLI step."
            done
            ...
            echo -e "${CYAN}[*] Session log saved to: ${WHITE}warstick-logs/audit_${TIMESTAMP}.txt${RESET}"
            echo -e "${CYAN}[*] Master log appended: ${WHITE}warstick-logs/warstick.log${RESET}"
        ...
        3)
            ...
            mkdir -p "$USB_ROOT/warstick-logs"
            TIMESTAMP=$(date +"%Y-%m-%d_%H-%M-%S")
            SESSION_LOG="$USB_ROOT/warstick-logs/custom_${TIMESTAMP}.txt"
            MASTER_LOG="$USB_ROOT/warstick-logs/warstick.log"
...
EOF
        ...
        6)
            ...
            open "http://127.0.0.1:9931" 2>/dev/null
            sleep 1
            ;;
        7)
            draw_banner
            echo -e "${CYAN}─── [HISTORICAL AUDIT STREAM LOGS] ────────────────────${RESET}\n"
            if [ -d "$USB_ROOT/warstick-logs" ] && [ "$(ls -A "$USB_ROOT/warstick-logs" 2>/dev/null)" ]; then
                echo -e "${WHITE}Available Session Logs:${RESET}"
                ls -1t "$USB_ROOT/warstick-logs" | head -n 10
                echo -e "\n${PINK}--- Displaying Latest Audit Stream ---${RESET}\n"
                LATEST_LOG=$(ls -1td "$USB_ROOT/warstick-logs"/* 2>/dev/null | head -n 1)
                cat "$LATEST_LOG"
            elif [ -f "$USB_ROOT/warstick-logs/warstick.log" ]; then
                cat "$USB_ROOT/warstick-logs/warstick.log"
            else
