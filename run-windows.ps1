...
. "$USB_ROOT\agent\lib\ui_common.ps1"
...
while ($true) {
    ...
    Write-Host "  [4] Custom Skills & External OSINT Lookups"
    Write-Host "  [5] Select / Switch Active LLM Model"
    ...
    switch ($choice) {
        "1" {
            ...
            $promptsFile = "$USB_ROOT\agent\prompts.txt"
        ...
        "2" {
            ...
            if (-not (Test-Path $promptsFile)) {
                New-Item -ItemType Directory -Path "$USB_ROOT\agent" -Force > $null
                @(
            ...
            Write-Host "$CYAN[*] Session log saved to: ${WHITE}agent\logs\audit_${timestamp}.txt$RESET"
            Write-Host "$CYAN[*] Master log appended: ${WHITE}agent\agent_log.txt$RESET"
        ...
        "3" {
            ...
            Write-Host "$CYAN[*] Log saved to: ${WHITE}agent\logs\custom_${timestamp}.txt$RESET"
            Write-Host "`n$PINK───────────────────────────────────────────────────────$RESET"
